...
def main():
    ...
    outputs_score = lasagne.layers.get_output(network['mp3'], deterministic=True) # change here for playing with a layer

    # prepare and compile prediction function
    print("Compiling prediction function...")
    pred_fn = theano.function([input_var], outputs_score, allow_input_downcast=True)
    print (lasagne.layers.get_output_shape(network['mp3']))   # change here for playing with a layer
    #>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
    # Comparator 
