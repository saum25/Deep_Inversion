...
def main():
    ...
    outputs_score = lasagne.layers.get_output(network['mp6'], deterministic=True) # enc_layer_name refers to the SVDNet(encoder) layer that needs to be inverted.

    # prepare and compile prediction function
    print("Compiling prediction function...")
    pred_fn = theano.function([input_var], outputs_score, allow_input_downcast=True)
    print (lasagne.layers.get_output_shape(network['mp6']))   # change here for layer name
    #>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
    # Comparator 
